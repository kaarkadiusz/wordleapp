...
export default function WordleGame() {
  ...
  function giveUp() {
    setGuess(-1);
    setOverlay({visible: true, text:`Prawidłowym słowem było ${wordToGuess.word.toUpperCase()}`});
  }
  function newGame() {
    setLoading(true);
      ...
      if (/^[a-ząćęłńóśźż]$/.test(inputChar) && guesses[guess].length < 5) {
        ...
        if (guesses[guess].length === 5) {
          ...
          if(result.every(val => val === 2)) {
            setGuess(-1);
            setOverlay({visible: true, text:`${wordToGuess.word.toUpperCase()} jest prawidłowe`})
          }
          else if (guess === 5) {
            setGuess(-1);
            setOverlay({visible: true, text:`Prawidłowym słowem było ${wordToGuess.word.toUpperCase()}`})
  ...
  const wordleRows = [];
  for (let i = 0; i < 6; i++) {
    wordleRows.push(<WordleRow word={guesses[i]} evaluation={evaluation[i]} key={i}/>)
  }
  return (
    <div>
      <header className="mb-1 flex flex-row justify-between">
        <button className={`bg-slate-200 hover:bg-slate-400 rounded py-1 px-2 text-black 
      ...
      <main
        className={`relative flex flex-col justify-center items-center w-fit h-fit 
        ${loading ? "blur-sm animate-pulse" : ""}`}
        >
        {wordleRows}
        <p className={`overlay absolute flex justify-center items-center w-full h-full bg-gray-900/80 text-4xl font-bold text-center
        ${overlay.visible ? "opacity-100" : "opacity-0"}`}>
          {overlay.text}
        </p>
      </main>
      <footer className={`flex justify-center items-center w-full h-16 bg-rose-500 mt-2 border-4 border-rose-700 rounded
